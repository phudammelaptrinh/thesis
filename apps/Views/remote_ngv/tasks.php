...
// Lấy danh sách dự án mà user nhận
$projects = $projectCtrl->getProjectsByUser($userId);
...
<div class="container-fluid mt-4">
    ...
    <!-- Chọn dự án -->
    <div class="mb-4">
        <label class="fw-semibold">Dự án:</label>
    ...
    <?php if ($currentProject): ?>
        <div class="card shadow-sm border-0">
            <div class="card-header bg-primary text-white fw-semibold d-flex justify-content-between align-items-center">
                <span>Dự án: <?= htmlspecialchars($currentProject['project_name']) ?></span>
                <span>Tiến độ: <?= $currentProject['progress'] ?? 0 ?>%</span>
            </div>
            <div class="card-body">
                <?php if (empty($tasks)): ?>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        window.changeProject = function (select) {
            window.location.href = `${window.APP_BASE_URL || ''}/apps/Views/ngv/index.php?page=tasks&project_id=${select.value}`;
        };

        window.deleteTask = function (taskId) {
            if (!confirm("Bạn có chắc muốn xóa công việc này không?")) return;
            fetch(`${window.APP_BASE_URL || ''}/apps/Controllers/TaskController.php?action=delete&task_id=${taskId}`)
                .then(res => res.json())
                .then(data => {
                    alert(data.message);
        ...
        window.openEditTaskModal = function (taskId) {
            ...
            fetch(`${window.APP_BASE_URL || ''}/apps/Controllers/TaskController.php?action=getTaskById&task_id=${taskId}`)
                .then(res => res.json())
                .then(data => {
